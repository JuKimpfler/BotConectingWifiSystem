...
public:
    ...
    void ingest(const TelemetryEntry_t *entry, uint8_t role);
    bool tick();           // Returns true if UI flush is due
    StreamStat *getStream(int idx);
    int         streamCount() const;
    StreamStat *findStream(const char *name, uint8_t role);
...
private:
    ...
    StreamStat *_getOrCreate(const char *name, uint8_t role);
};
